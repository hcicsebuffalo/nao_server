...
documents = []
for file in os.listdir("LLM_code/docs"):
    if file.endswith('.txt'):
        text_path = "LLM_code/docs/" + file
        loader = TextLoader(text_path)
        documents.extend(loader.load())
"""for file in os.listdir("LLM_code/data"):
    if file.endswith(".pdf"):
        pdf_path = "LLM_code/data/" + file
        loader = PyPDFLoader(pdf_path)
        documents.extend(loader.load())"""
text_splitter = CharacterTextSplitter(chunk_size=4000, chunk_overlap=20)
documents = text_splitter.split_documents(documents)
embeddings = HuggingFaceEmbeddings(model_name="BAAI/bge-reranker-large")#"hkunlp/instructor-base")
